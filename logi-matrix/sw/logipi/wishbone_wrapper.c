/*!@file  wishbone_wrapper.c
 * @brief wraps wishbone on top of SPI
 *
...
/* used for communication protocol on FPGA:
 *
 *   In this protocol the first 14bits send over SPI are the address and then 
 *   a bit indicate write or read operation and one bit indicates to increment 
 *   address or not for following data
 */
...
/* helper for wishbone: SPI wrappers for direct communication
 */
void spi_close(void) ;
int spi_init(void) ;
...
//use spi to communicate a send buffer and receive buffer
int spi_transfer(unsigned char * send_buffer, unsigned char * receive_buffer, unsigned int size)
{
	int ret ;
...
//set or change max speed
int set_speed(unsigned long speed_arg){
	
	int ret;
    //in case the device is not open
	wishbone_init();
	speed = speed_arg;
