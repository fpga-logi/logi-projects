...
unsigned int wishbone_write_noinc(unsigned char * buffer, unsigned int length, unsigned int address){
	unsigned int tr_size = 0, count = 0 ;
	if(fd == 0){
		spi_init();
	}
	while(count < length){
                tr_size = (length-count) < 4094 ? (length-count) : 4094 ;
		if(logipi_write(((address+count)), &buffer[count], tr_size, 0) < 0) return 0;
		count = count + tr_size ;
        }

	return count ;
}
unsigned int wishbone_read_noinc(unsigned char * buffer, unsigned int length, unsigned int address){
	unsigned int tr_size = 0, count = 0 ;
	if(fd == 0){
		spi_init();
	}
	while(count < length){
		tr_size = (length-count) < 4094 ? (length-count) : 4094 ;
		if(logipi_read(((address+count)), &buffer[count], tr_size, 0) < 0) return 0 ;
		count = count + tr_size ;
	}
	return count ;
}
