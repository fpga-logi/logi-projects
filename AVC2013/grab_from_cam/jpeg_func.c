...
int write_jpegfile(char * frame, unsigned short width, unsigned short height, int nbChan, FILE * fd, int quality)
{
  JSAMPROW row_ptr[1];
  ...
  char *line, *image;
  int y, x, line_width;
  line =(char *) malloc((width) *nbChan* sizeof(char));
  if (!line)
    return 0;
