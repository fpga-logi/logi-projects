...
class AvcPlatform(object):
	...
	def getBlobPos(self):
        ...
        def computebearing(self, p1,p2):
                lat1,lon1,alt1=Point(p2)
                lat2,lon2,alt2=Point(p1)
                lon1, lat1, lon2, lat2 = map(radians, [lon1, lat1, lon2, lat2])
                # haversine formula
                dlon = lon2 - lon1
                dlat = lat2 - lat1
...
if __name__ == "__main__":
	...
	i = 0
        while True:
                ...
                #print  distance.distance(robot.waypoints[0],robot.waypoints[1]).meters, "meters"
                #print "haversine dist =", robot.computedist(robot.waypoints[0], robot.waypoints[1])*1000 , "meters"
                #print "haversine heading =", degrees(robot.computebearing(robot.waypoints[0], robot.waypoints[1])) , "degrees"
                pos = robot.waypoints[0]
                cpos= robot.waypoints[0]
                rpos=Point("40.575265,-75.755850")
                ...
                print  distance.distance(robot.waypoints[0],robot.waypoints[1]).meters, "meters"
                print "haversine dist =", robot.computedist(rpos, cpos)*1000 , "meters"
                print "haversine heading =", degrees(robot.computebearing(rpos, cpos)) , "degrees"
